...
export const getComments = async(req,res,next) =>{
  if(!req.user.isAdmin)return next(errorHandler(403,'You are not authorized to access all comments'))
  try {
    ...
    res.status(200).json({ comments, totalNoOfComments, lastMonthComments });
  } catch (error) {
    next(error);
  }
}